...
%filename = 'Par_NO_Ideal_Animated5.gif';       % Specify the output file name
%%gifc = 1 ;                                   %1==yes or 0==no ==> creating .gif animation file
...
S = 1  ;%+  0.19*sin(3.*wtr)   -  0.50 ;                  %Bs=S*Bm*sin(w*t-2*pi/3);
...
%ylim([-2 2]);
xlim([0 max(wtd)]);grid on; grid minor;
ax = gca;
...
for ii=1:n        
    a(ii)=plot([0 Nx(ii)],[0 Ny(ii)],'r','Marker','o','LineWidth',1.5,'Visible','off');    
    b(ii)=plot([0 Rx(ii)],[0 Ry(ii)],'k','Marker','o','LineWidth',1.0,'Visible','off');
    c(ii)=plot([0 Sx(ii)],[0 Sy(ii)],'b','Marker','o','LineWidth',1.0,'Visible','off');
    d(ii)=plot([0 Tx(ii)],[0 Ty(ii)],'m','Marker','o','LineWidth',1.0,'Visible','off');
end
%%
toc;tic;
%%
